...
export class Migration20250511100618 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table \`user\` (\`id\` int unsigned not null auto_increment primary key, \`username\` varchar(255) not null, \`password\` varchar(255) not null, \`role\` varchar(255) not null) default character set utf8mb4 engine = InnoDB;`);

    this.addSql(`create table \`tournament\` (\`id\` varchar(36) not null, \`name\` varchar(255) not null, \`city\` varchar(255) not null, \`start_date\` datetime not null, \`end_date\` datetime not null, \`user_id_id\` int unsigned not null, primary key (\`id\`)) default character set utf8mb4 engine = InnoDB;`);
    this.addSql(`alter table \`tournament\` add index \`tournament_user_id_id_index\`(\`user_id_id\`);`);
    ...
    this.addSql(`alter table \`tournament\` add constraint \`tournament_user_id_id_foreign\` foreign key (\`user_id_id\`) references \`user\` (\`id\`) on update cascade;`);
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(`alter table \`tournament\` drop foreign key \`tournament_user_id_id_foreign\`;`);
